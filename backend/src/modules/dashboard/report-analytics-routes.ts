...
export async function reportAnalyticsRoutes(app: FastifyInstance): Promise<void> {
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // 10. GET /api/v1/reports/kpi-history — Phase 5 Báo cáo KPI 30 ngày 2026-08-21.
  // "Task hoàn thành/ngày" = tổng (newLeads + sent) trong ShiftReport.metrics của
  // TẤT CẢ báo cáo ca nộp trong ngày đó (1 user có thể nộp nhiều ca/ngày → cộng dồn).
  // So với Organization.dailyTaskTarget để tô Đạt/Chưa đạt/Không có dữ liệu.
  // ─────────────────────────────────────────────────────────────────────────
  app.get('/api/v1/reports/kpi-history', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const user = request.user!;
      const query = request.query as QueryParams;
      ...
      if (targetUserId !== user.id) {
        ...
        if (!scope.canViewAll && !scope.visibleUserIds.includes(targetUserId)) {
          return reply.status(403).send({ error: 'Không có quyền xem KPI của người này' });
        }
      }

      const org = await prisma.organization.findUnique({
        where: { id: user.orgId },
        select: { dailyTaskTarget: true },
      });
      const dailyTaskTarget = org?.dailyTaskTarget ?? 10;

      const now = new Date();
      const dateKeys: string[] = [];
      for (let i = days - 1; i >= 0; i--) {
        dateKeys.push(reportDateVN(new Date(now.getTime() - i * 86400000)));
      }
      ...
      const reports = await prisma.shiftReport.findMany({
        where: {
          orgId: user.orgId,
          userId: targetUserId,
          reportDate: { in: dateKeys },
        },
        select: { reportDate: true, metrics: true },
      });

      const countByDate = new Map<string, number>();
      for (const r of reports) {
        const m = (r.metrics ?? {}) as { newLeads?: number; sent?: number };
        const c = N(m.newLeads) + N(m.sent);
        countByDate.set(r.reportDate, (countByDate.get(r.reportDate) ?? 0) + c);
      }
      ...
      const history = dateKeys.map((date) => {
        ...
        return {
          date,
          count,
          achieved: hasData ? count >= dailyTaskTarget : null, // null = không có dữ liệu (xám)
        };
      });

      const daysWithData = history.filter((h) => h.achieved !== null);
      const avgPerDay = daysWithData.length > 0
        ? Math.round((daysWithData.reduce((a, h) => a + h.count, 0) / daysWithData.length) * 10) / 10
        : 0;
      const achievedDays = daysWithData.filter((h) => h.achieved).length;

      // % xu hướng: so trung bình nửa đầu vs nửa sau cửa sổ (chỉ tính ngày có dữ liệu).
      const mid = Math.floor(history.length / 2);
      const firstHalf = history.slice(0, mid).filter((h) => h.achieved !== null);
      const secondHalf = history.slice(mid).filter((h) => h.achieved !== null);
      const avgFirst = firstHalf.length > 0 ? firstHalf.reduce((a, h) => a + h.count, 0) / firstHalf.length : 0;
      const avgSecond = secondHalf.length > 0 ? secondHalf.reduce((a, h) => a + h.count, 0) / secondHalf.length : 0;
      const trendPct = avgFirst > 0 ? Math.round(((avgSecond - avgFirst) / avgFirst) * 1000) / 10 : null;
      ...
      return reply.send({
        ...
        dailyTaskTarget,
        history,
        summary: { avgPerDay, achievedDays, totalDaysWithData: daysWithData.length, trendPct },
      });
    } catch (err) {
      logger.error('[reports] kpi-history error:', err);
      return reply.status(500).send({ error: 'Failed to fetch KPI history' });
    }
  });
}
