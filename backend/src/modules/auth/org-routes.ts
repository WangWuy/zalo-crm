...
export async function orgRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.put(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      // Phase 5 Báo cáo KPI 30 ngày 2026-08-21 — số task/ngày mặc định để tô Đạt/Chưa đạt.
      if (body.dailyTaskTarget !== undefined) {
        const n = Number(body.dailyTaskTarget);
        if (!Number.isFinite(n) || n < 1 || n > 1000) {
          return reply.status(400).send({ error: 'dailyTaskTarget phải là số nguyên từ 1 đến 1000' });
        }
        data.dailyTaskTarget = Math.round(n);
      }
