...
async function bootstrap() {
  ...
  // Quick Note Tag (Phase 3 TheLab) — chip 1-chạm chèn nhanh text vào ô ghi chú
  const { registerQuickNoteTagRoutes } = await import('./modules/tags/quick-note-tag-routes.js');
  await app.register(registerQuickNoteTagRoutes, { prefix: '/api/v1/quick-note-tags' });
  // Phase 1 Ca làm việc & Báo cáo ca (2026-08-21, upgrade-crm implementation_plan.md)
  const { registerShiftScheduleRoutes } = await import('./modules/shift/shift-schedule-routes.js');
  await app.register(registerShiftScheduleRoutes, { prefix: '/api/v1/shifts/schedules' });
  const { registerShiftReportRoutes } = await import('./modules/shift/shift-report-routes.js');
  await app.register(registerShiftReportRoutes, { prefix: '/api/v1/shifts' });
  // Phase 2 Data Pool tự phục vụ (2026-08-21, upgrade-crm implementation_plan.md)
  const { registerLeadPoolClaimRoutes } = await import('./modules/lead-pool-claim/claim-quota-routes.js');
  await app.register(registerLeadPoolClaimRoutes, { prefix: '/api/v1/lead-pool' });
  ...
  // Lead Pool → registered by extension bundle.
  // Facebook Lead Ads ingestion (2026-08-21, core rewrite — xem docs/upgrade-crm/implementation_plan.md)
  const { facebookWebhookRoutes } = await import('./modules/integrations/facebook/facebook-webhook-routes.js');
  await app.register(facebookWebhookRoutes);
  const { facebookPageRoutes } = await import('./modules/integrations/facebook/facebook-page-routes.js');
  await app.register(facebookPageRoutes);
