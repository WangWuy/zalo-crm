...
export function useChat() {
  ...
  // Pagination "load more" tin cũ (cuộn lên) — page hiện đã fetch + còn tin cũ hơn không.
  const messagesPage = ref(1);
  const hasMoreMessages = ref(true);
  const loadingOlderMsgs = ref(false);
  ...
  async function fetchMessages(convId: string) {
    ...
    try {
      ...
      if (isConvCurrent(convId)) {
        messagesPage.value = 1;
        hasMoreMessages.value = list.length < (res.data.total ?? 0);
      }
  ...
  // Cuộn lên đầu thread → fetch trang tin cũ hơn (page+1) và prepend vào messages.value.
  // Trước fix (2026-07-16): FE chỉ gọi fetchMessages(limit:100) 1 lần, không track page/total
  // → conv nào >100 tin thì tin cũ hơn 100 tin gần nhất KHÔNG BAO GIỜ xem lại được.
  async function loadOlderMessages(convId: string) {
    if (loadingOlderMsgs.value || !hasMoreMessages.value) return;
    if (!isConvCurrent(convId)) return;
    loadingOlderMsgs.value = true;
    try {
      ...
      const older = list.filter(m => !existingIds.has(m.id));
      if (older.length > 0) {
      ...
      hasMoreMessages.value = messages.value.length < (res.data.total ?? 0);
    } catch (err) {
      console.error('Failed to fetch older messages:', err);
    } finally {
      loadingOlderMsgs.value = false;
    }
  }
