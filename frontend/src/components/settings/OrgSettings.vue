    ...
    <v-card variant="outlined" class="pa-4">
      ...
      <div class="d-flex align-center mb-3" style="gap: 12px;">
        ...
        <v-text-field
          ...
          placeholder="/brand/logo02.png hoặc https://..."
          :disabled="!authStore.isOwner || saving"
          variant="outlined"
          density="compact"
          hide-details
      ...
      <v-divider class="my-4" />
      <div class="text-subtitle-2 mb-1">Báo cáo KPI</div>
      <v-text-field
        v-model.number="dailyTaskTarget"
        type="number"
        min="1"
        max="1000"
        label="Số task/ngày mục tiêu"
        :disabled="!authStore.isOwner || saving"
        variant="outlined"
        class="mb-3"
        hint="Điều chỉnh theo thực tế đội nhóm — đây là số kỹ thuật mặc định, không phải chỉ tiêu chính thức. Dùng để tô Đạt/Chưa đạt trên biểu đồ KPI 30 ngày."
        persistent-hint
      />
...
const canSave = computed(() => {
  ...
  return (
    ...
    dailyTaskTarget.value !== original.value.dailyTaskTarget
  );
});
...
async function fetchOrg() {
  try {
    ...
    dailyTaskTarget.value = res.data.dailyTaskTarget ?? 10;
    original.value = {
      name: orgName.value,
      ...
      dailyTaskTarget: dailyTaskTarget.value,
    };
  } catch {
...
async function handleSave() {
  ...
  try {
    ...
    copyright.value = res.data.copyright ?? '';
    emailDomain.value = res.data.emailDomain ?? '';
    dailyTaskTarget.value = res.data.dailyTaskTarget ?? dailyTaskTarget.value;
    original.value = {
      name: orgName.value,
      timezone: timezone.value,
      logoUrl: logoUrl.value,
