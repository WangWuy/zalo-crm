...
<template>
  ...
  >
    ...
    <template v-else>
      ...
      <!-- ════════ Messages ════════ -->
      <div ref="messagesContainer" class="messages chat-messages-area" :class="{ 'is-virtual-mode': isVirtualConv }" @scroll="onMessagesScroll">
        <v-progress-linear v-if="loading" indeterminate color="primary" class="mb-2" />
        <div v-if="loadingOlderMessages" class="text-center text-caption text-medium-emphasis py-2">Đang tải tin nhắn cũ hơn...</div>
...
// Cuộn lên gần đầu container → xin parent load thêm tin cũ hơn (page+1).
// Giữ nguyên vị trí scroll sau khi prepend (không nhảy về đáy) — so sánh
// scrollHeight trước/sau trong watch bên dưới.
const SCROLL_TOP_THRESHOLD = 80;
function onMessagesScroll() {
  const el = messagesContainer.value;
  if (!el) return;
  if (el.scrollTop <= SCROLL_TOP_THRESHOLD && props.hasMoreMessages && !props.loadingOlderMessages) {
    emit('load-older');
  }
}

// Khi messages thêm (tin mới đến ở CUỐI) → scroll mượt xuống đáy.
// Khi tin cũ được PREPEND (load-older) → giữ nguyên vị trí xem hiện tại thay vì
// nhảy xuống đáy: bù lại scrollTop bằng đúng độ tăng scrollHeight.
watch(() => props.messages.length, async (newLen, oldLen) => {
  const el = messagesContainer.value;
  const prevScrollHeight = el?.scrollHeight ?? 0;
  const wasNearTop = !!el && el.scrollTop <= SCROLL_TOP_THRESHOLD;
  await nextTick();
  if (wasNearTop && newLen > oldLen && el) {
