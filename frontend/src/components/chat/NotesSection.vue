...
<template>
  <section class="notes-section">
    ...
    <!-- Quick Note Tags: chip 1-chạm append vào note draft -->
    <div v-if="quickTags.length" class="quick-tags-row">
      <button
        v-for="t in quickTags"
        :key="t.id"
        class="quick-tag-chip"
        type="button"
        @click="appendQuickTag(t.label)"
      >
        {{ t.label }}
      </button>
    </div>
...
import { useQuickNoteTags } from '@/composables/use-quick-note-tags';
...
const { tags: quickTags, loadQuickNoteTags } = useQuickNoteTags();
onMounted(() => { void loadQuickNoteTags(); });

function appendQuickTag(label: string) {
  rootDraft.value = rootDraft.value.trim() ? `${rootDraft.value.trim()} ${label}` : label;
  nextTick(() => {
