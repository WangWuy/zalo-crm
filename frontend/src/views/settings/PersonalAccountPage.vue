...
<template>
  <div class="account-page">
    ...
    <template v-if="authStore.user">
      ...
      <!-- Card 3.5: Cài đặt cá nhân (Night mode + dialer) -->
      <div class="card">
        <div class="card-head"><h3>Cài đặt cá nhân</h3></div>
        <div class="card-pad fields">
          <div class="frow">
            <div class="flabel">Night Priority Mode</div>
            <div class="fval night-row">
              <label class="switch">
                <input type="checkbox" v-model="nightModeEnabled" @change="onNightModeChange" :disabled="prefsBusy" />
                <span class="switch-track"><span class="switch-thumb"></span></span>
              </label>
              <span class="night-desc">Ưu tiên nhận lead ngoài giờ hành chính</span>
            </div>
          </div>
          <div class="frow">
            <div class="flabel">Phương thức gọi</div>
            <div class="fval">
              <select v-model="dialerMethod" @change="onDialerMethodChange" class="finput dialer-select" :disabled="prefsBusy">
                <option value="tel">Mặc định (tel:)</option>
                <option value="custom">Tuỳ chỉnh (URI scheme)</option>
              </select>
              <input
                v-if="dialerMethod === 'custom'"
                v-model="dialerScheme"
                @change="onDialerSchemeChange"
                class="finput dialer-scheme"
                placeholder="vd: zoiper://"
                :disabled="prefsBusy"
              />
            </div>
          </div>
...
import { getUserPreference, setUserPreference } from '@/composables/use-user-preference';
import { useDialer, DIALER_PREFERENCE_KEY, type DialerPreference } from '@/composables/use-dialer';
...
// ── Cài đặt cá nhân: Night Priority Mode + Dialer preference ──
const NIGHT_MODE_KEY = 'night-mode.enabled';
const { save: saveDialerPref } = useDialer();
const prefsBusy = ref(false);
const nightModeEnabled = ref(false);
const dialerMethod = ref<DialerPreference['method']>('tel');
const dialerScheme = ref('');

onMounted(async () => {
  try {
    const [nightMode, dialerPref] = await Promise.all([
      getUserPreference<boolean>(NIGHT_MODE_KEY, false),
      getUserPreference<DialerPreference>(DIALER_PREFERENCE_KEY, { method: 'tel' }),
    ]);
    nightModeEnabled.value = nightMode;
    dialerMethod.value = dialerPref.method;
    dialerScheme.value = dialerPref.customUriScheme ?? '';
  } catch {
...
async function onNightModeChange() {
  ...
  try {
    await setUserPreference(NIGHT_MODE_KEY, nightModeEnabled.value);
    toast.success(nightModeEnabled.value ? 'Đã bật Night Priority Mode' : 'Đã tắt Night Priority Mode');
  } catch {
    nightModeEnabled.value = !nightModeEnabled.value;
    toast.error('Không thể lưu cài đặt Night Mode');
  } finally {
...
async function saveDialerSettings() {
  prefsBusy.value = true;
  try {
    await saveDialerPref({
      method: dialerMethod.value,
      customUriScheme: dialerMethod.value === 'custom' ? dialerScheme.value.trim() : undefined,
    });
    toast.success('Đã lưu phương thức gọi');
  } catch {
    toast.error('Không thể lưu phương thức gọi');
  } finally {
    prefsBusy.value = false;
  }
}
function onDialerMethodChange() {
  void saveDialerSettings();
}
function onDialerSchemeChange() {
  void saveDialerSettings();
}
</script>
...
/* Night mode toggle + dialer */
.night-row { display: flex; align-items: center; gap: 12px; }
.switch { position: relative; display: inline-block; width: 40px; height: 22px; flex-shrink: 0; }
.switch input { opacity: 0; width: 0; height: 0; }
.switch-track {
  position: absolute; inset: 0; background: var(--line, #e7eaf0);
...
.switch-thumb {
  ...
  box-shadow: 0 1px 3px rgba(20,26,36,.2);
}
.switch input:checked + .switch-track { background: var(--brand, #1786be); }
.switch input:checked + .switch-track .switch-thumb { transform: translateX(18px); }
.switch input:disabled + .switch-track { opacity: .6; cursor: default; }
.night-desc { font-size: 13px; color: var(--ink-3, #6b7488); }
.dialer-select { max-width: 220px; margin-right: 10px; }
.dialer-scheme { max-width: 160px; display: inline-block; }
