...
const routes: RouteRecordRaw[] = [
  ...
  // ════════ Module Báo cáo — shell + 7 màn (2026-06-17) ════════
  {
    ...
    children: [
      ...
      { path: 'shift-history', name: 'Reports.ShiftHistory', component: () => import('@/views/reports/ShiftHistoryReport.vue'), meta: { resource: 'engagement_score' } },
      ...eeReportsChildren,
    ],
  },
  ...
  // ════════ NEW Settings — 6-group sidebar layout ════════
  {
    ...
    children: [
      ...
      // Facebook Lead Ads ingestion (2026-08-21, core rewrite)
      { path: 'channels/facebook',         name: 'Settings.FacebookPages',   component: () => import('@/views/settings/FacebookPagesSettings.vue'), meta: { resource: 'settings' } },
...
router.beforeEach(async (to, _from, next) => {
  ...
  if (to.meta.requiresAuth) {
    ...
    if (!authStore.user) {
      ...
      // Only bounce to /login if the token itself got cleared (real auth
      // failure). A transient network/server error during fetchProfile()
      // shouldn't log the user out while their token is still valid.
      if (!authStore.token) {
        return next('/login');
      }
    }
