...
check_migrations() {
  [ -f scripts/check-migrations.sh ] || return 0
  log "Rà migration đang chờ áp dụng (tìm lệnh DROP/đổi kiểu/NOT NULL không default)…"
  if ! DB="$DB" DBUSER="$DBUSER" DBNAME="$DBNAME" scripts/check-migrations.sh; then
    if [ "${FORCE_MIGRATE:-0}" = "1" ]; then
      warn "Phát hiện migration rủi ro nhưng FORCE_MIGRATE=1 → tiếp tục theo yêu cầu."
    else
      die "Dừng lại vì migration có rủi ro (xem chi tiết ở trên). Backup xong rồi chạy lại với FORCE_MIGRATE=1 nếu chắc chắn."
    fi
  fi
}
